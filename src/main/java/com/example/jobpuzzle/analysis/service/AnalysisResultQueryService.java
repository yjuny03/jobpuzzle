...
public class AnalysisResultQueryService {
 private final AnalysisCaseRepository cases; private final AnalysisInputSnapshotRepository snapshots; private final JobPostingAnalysisRepository postings; private final CandidateMaterialAnalysisRepository candidates; private final GuideContextResultRepository guides; private final GuideContextChunkRepository chunks; private final ReadinessResultRepository readiness; private final MatchAnalysisResultRepository matches; private final ActionPlanRepository actions; private final QuestionSetRepository sets; private final InterviewQuestionRepository questions; private final CustomizedAnalysisInputMapper mapper;
 @Transactional(readOnly=true) public AnalysisResultResponse getResult(Long userId, Long caseId) {
  ...
  JobPostingAnalysis p=postings.findBySnapshot_SnapshotId(s.getSnapshotId()).orElseThrow(this::integrity); CandidateMaterialAnalysis m=candidates.findBySnapshot_SnapshotId(s.getSnapshotId()).orElseThrow(this::integrity);
