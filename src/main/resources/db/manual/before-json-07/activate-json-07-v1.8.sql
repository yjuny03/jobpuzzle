-- 목적: 관점별 점수(categoryScores)가 왜 그렇게 나왔는지 서술해주는 categoryScoreReasons를 추가한다.
-- 프론트에서 레이더 차트 옆에 관점별 근거를 보여주기 위함. buildPrompt()가 이제 각 답변의
-- evaluationDetail(관점별 score+comment)을 같이 넘겨주므로, 그 comment를 근거로 작성하게 한다.
...
SELECT 'PT-REPORT-001', '최종 면접 리포트', 'v1.8', 'JSON-07',
...
,
'["JSON 외 텍스트·Markdown·코드블록 출력 금지","입력에 없는 점수·태그·근거 생성 금지","미답변 질문 평가 금지","합격 가능성·채용 여부로 해석되는 표현 금지","제출하지 않은 자료에 대한 보완 제안 생성 금지","서술형 텍스트에 영문 필드명·enum 값·score 같은 영단어 그대로 노출 금지","overallAssessment를 [잘한 점]/[부족한 점]/[총평] 3개 섹션 형식 외로 작성 금지","weaknessTagSummary에 reason 없이 tag·count만 반환 금지","categoryScores가 null인 관점에 categoryScoreReasons 값 생성 금지"]',
TRUE, CURRENT_TIMESTAMP
WHERE NOT EXISTS (
    SELECT 1 FROM prompt_template WHERE target_json = 'JSON-07' AND version = 'v1.8'
);

UPDATE prompt_template SET is_active = TRUE WHERE target_json = 'JSON-07' AND version = 'v1.8';
