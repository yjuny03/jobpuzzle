-- 목적: v1.5는 "[부족한 점]"을 weaknessTags 태그별 세로 목록으로 요청했는데,
-- 실제 의도는 세션 전체 부족한 점을 [잘한 점]/[총평]과 같은 서술형 문단으로 쓰는 것이었다(점수 배지는 유지).
-- 태그별 이유를 붙이라는 요청은 overallAssessment가 아니라 weaknessTagSummary 항목(JSON-07 응답의 별도 필드)에
-- 대한 것이었으므로, weaknessTagSummary에 reason 필드를 추가한 v1.6을 활성화한다.
...
SELECT 'PT-REPORT-001', '최종 면접 리포트', 'v1.6', 'JSON-07',
...
- reason에는 그 태그가 어떤 답변의 score·summary 근거로 나오게 됐는지 구체적으로 설명한다.
...
,
'["JSON 외 텍스트·Markdown·코드블록 출력 금지","입력에 없는 점수·태그·근거 생성 금지","미답변 질문 평가 금지","합격 가능성·채용 여부로 해석되는 표현 금지","제출하지 않은 자료에 대한 보완 제안 생성 금지","서술형 텍스트에 영문 필드명·enum 값 그대로 노출 금지","overallAssessment를 [잘한 점]/[부족한 점]/[총평] 3개 섹션 형식 외로 작성 금지","weaknessTagSummary에 reason 없이 tag·count만 반환 금지"]',
TRUE, CURRENT_TIMESTAMP
WHERE NOT EXISTS (
    SELECT 1 FROM prompt_template WHERE target_json = 'JSON-07' AND version = 'v1.6'
);

UPDATE prompt_template SET is_active = TRUE WHERE target_json = 'JSON-07' AND version = 'v1.6';
