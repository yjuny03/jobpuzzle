-- 목적: v1.4까지의 overallAssessment는 강점·약점·원인·예외사례·결론이 한 문단에 뒤섞여
-- 정보 밀도가 너무 높았다. [잘한 점]/[부족한 점]/[총평] 3개 섹션으로 나누고,
-- 부족한 점 섹션은 세션 종합 점수(overallScore)를 제목에 인라인으로 붙인 뒤
-- weaknessTags를 태그별 세로 목록(태그 + 근거 설명)으로 나열하도록 바꾼 v1.5를 활성화한다.
...
SELECT 'PT-REPORT-001', '최종 면접 리포트', 'v1.5', 'JSON-07',
...
- 그 아래에 입력 answer들의 weaknessTags를 모두 모아 태그별로 한 줄씩 세로 목록으로 나열한다.
  각 줄은 "- 태그명: 설명" 형식이며, 설명에는 그 태그가 어떤 답변의 score·summary 근거로 나왔는지 구체적으로 쓴다.
- 같은 태그가 여러 답변에서 반복됐으면 그 근거들을 한 줄로 종합해서 쓰고, 같은 태그를 중복으로 나열하지 않는다.
- 입력 answer들에 weaknessTags가 하나도 없으면 목록 대신 "특별히 부족한 부분은 확인되지 않았습니다." 한 줄만 쓴다.
...
,
'["JSON 외 텍스트·Markdown·코드블록 출력 금지","입력에 없는 점수·태그·근거 생성 금지","미답변 질문 평가 금지","합격 가능성·채용 여부로 해석되는 표현 금지","제출하지 않은 자료에 대한 보완 제안 생성 금지","서술형 텍스트에 영문 필드명·enum 값 그대로 노출 금지","overallAssessment를 [잘한 점]/[부족한 점]/[총평] 3개 섹션 형식 외로 작성 금지"]',
TRUE, CURRENT_TIMESTAMP
WHERE NOT EXISTS (
    SELECT 1 FROM prompt_template WHERE target_json = 'JSON-07' AND version = 'v1.5'
);

UPDATE prompt_template SET is_active = TRUE WHERE target_json = 'JSON-07' AND version = 'v1.5';
