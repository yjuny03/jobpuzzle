...
(function () {
  ...
  function showRetryProgress(status, delaySeconds) {
    ...
    stages.forEach(function (stage) {
      ...
      var row = el('div', 'progress-step progress-step--' + value);
      row.appendChild(el('span', 'progress-step__mark', value === 'SUCCEEDED' ? '✓' : '·'));
      var copy = el('div');
      copy.appendChild(el('strong', null, stage[1]));
  ...
  function canRetryAutomatically(status) {
    var serverAllowsRetry = status.retryable === true;
  ...
  function scheduleAutomaticRetry(status) {
    ...
    var delaySeconds = Number(status.retryAfterSeconds) || Math.min(3 * Math.pow(2, state.automaticRetries), 12);
  ...
  function runAnalysis(automaticRetry) {
    if (state.running) return;
    state.running = true;
    var prepare = automaticRetry === true
      ? Promise.resolve()
    ...
    prepare
      ...
      .catch(function (error) {
        ...
        analysisApi('/status').then(function (status) {
          if (automaticRetry === true && canRetryAutomatically(status)) {
            scheduleAutomaticRetry(status);
            return;
          }
          renderProgress(status, automaticRetry === true ? null : error);
        }).catch(function () {
          renderProgress({ analysisCaseStatus: 'FAILED' }, automaticRetry === true ? null : error);
        });
      });
  }
