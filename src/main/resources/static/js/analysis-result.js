...
(function () {
  ...
  function renderCta(questionCount) {
    var card = node('section', 'result-cta');
    card.appendChild(node('h2', null, '분석을 연습으로 연결해 보세요'));
    card.appendChild(node('p', null, questionCount
      ? '생성된 ' + questionCount + '개 질문으로 맞춤 면접을 시작할 수 있어요.'
      : '자료를 보완한 뒤 새로운 맞춤 분석을 진행해 보세요.'));
    var link = node('a', 'result-cta__button', questionCount ? '맞춤 면접 시작' : '면접 준비로 이동');
    link.href = questionCount
  ...
  function render(result) {
    ...
    side.appendChild(renderCta(questions.length));
